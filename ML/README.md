...
You are provided with a unified, curated dataset of **scanned business documents** (merged from SROIE, Find-It-Again, and CORD). Each document may contain:
...
Your task is to **automatically extract structured fields** from documents. To pass the automated evaluation, your pipeline must successfully extract the following mandatory fields:
...
Identify suspicious or unusual documents (focusing on the **Find-It-Again** subset). You must output an `is_forged` prediction (1 for forged, 0 for genuine). Look for:
...
**Part B: Basic Web UI**
Develop a simple web-based dashboard (e.g., using Streamlit, Flask, or Gradio) to display your findings.
...
Your pipeline will be rigorously tested by the **DocFusion Autograder Harness**, measuring speed, memory, and accuracy.

**Part A: The Interface Contract**
You must implement a `solution.py` file containing the `DocFusionSolution` class. This class must handle both training and prediction:
...
* Strict dependency management (e.g., `uv`, `poetry`, or `requirements.txt`).
...
These are **optional extensions** but will strengthen your manual evaluation score.

* **Containerization:** Create a **Dockerfile** to containerize your project (Model + UI). Ensure all OCR dependencies and libraries are included.
* **Cloud Deployment:** Deploy your application on a VPS or cloud platform. Provide a **public endpoint** or demo link.
...
Your submission will be evaluated using a hybrid approach: **Automated Scoring (via Harness)** and **Manual Review (by Judges)**.

#### **What to Submit:**

1. **`solution.py`**: Must contain the `DocFusionSolution` class exactly as specified in the interface contract.
2. **Dependencies**: A clear `pyproject.toml`, `requirements.txt`, or `uv` lockfile. Python 3.13+ is required.
3. **Jupyter Notebooks**: Documenting your Level 1 EDA, model training logic, and extraction experiments.
4. **Web UI Code**: Scripts to run your Level 3 dashboard (e.g., `app.py`).
5. **Documentation**: A comprehensive README explaining your approach, architecture, and instructions for running your UI and Docker containers.

#### **Evaluation Breakdown:**

| Category | Evaluation Method | Key Metrics |
| --- | --- | --- |
| **Functional Accuracy** | **Automated (Harness)** | Extraction Accuracy (Vendor, Date, Total) and Classification F1 Score (`is_forged`). |
| **Efficiency & Performance** | **Automated (Harness)** | Throughput (docs/sec), Per-document latency, Peak memory usage, Model size footprint. |
| **Engineering Quality** | **Manual Review** | Code organization, modularity, reproducibility, and clarity of documentation. |
| **Usability & Robustness** | **Manual Review** | Dashboard UX, visual accuracy of bounding boxes, and error handling for bad inputs. |
| **Bonus: Deployment** | **Manual Review** | Successful Docker build, active public URL, and intelligent assistance features. |

**Final Note:**
Creativity, clarity, and real-world thinking are strongly encouraged. The strongest solutions will not only pass the automated benchmarks but will demonstrate **production awareness, efficiency, and thoughtful engineering design**.
